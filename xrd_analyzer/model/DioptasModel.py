...
class DioptasModel:
    ...
    def add_configuration(self) -> None:
        ...
        if self.current_configuration.calibration_model.is_calibrated:
            xrd_config_folder = os.path.join(os.path.expanduser("~"), ".XrdAnalyzer")
            if not os.path.isdir(xrd_config_folder):
                os.mkdir(xrd_config_folder)
            self.current_configuration.calibration_model.save(
                os.path.join(xrd_config_folder, "transfer.poni")
            )
