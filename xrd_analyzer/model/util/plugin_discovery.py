...
MASK_PLUGIN_ENTRY_POINT_GROUP = "xrd_analyzer.plugins.masks"
USER_PLUGIN_DIR = Path.home() / ".xrd_analyzer" / "plugins" / "masks"
...
def _discover_from_directory(plugin_dir: Path) -> list[type[MaskPluginBase]]:
    ...
    for py_file in sorted(plugin_dir.glob("*.py")):
        if py_file.name.startswith("_"):
            continue
        try:
            module_name = f"xrd_analyzer_user_mask_plugin_{py_file.stem}"
